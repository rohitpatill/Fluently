...
- **This backend serves the website AND the native Android app** (same API, same session JWT). The app can't use the HttpOnly cookie, so `app/deps.py` also accepts `Authorization: Bearer` (HTTP) and `?token=` (WebSocket). Keep every auth transport inside `deps.py` — never per-router. Prod `CORS_ALLOWED_ORIGINS` must include `https://localhost` (the app's WebView origin). App-side details: `frontend/android/frontendAndroidContext.md`.
