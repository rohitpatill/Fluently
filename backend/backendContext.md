...
| `requirements.txt` | Pinned-ish deps: fastapi, **pymongo, dnspython**, langchain (+openai/anthropic/google-genai), rank-bm25, pytest, httpx, **google-auth, PyJWT, itsdangerous** (Google OAuth + JWT session), **cryptography** (Fernet — encrypts users' BYO API keys). (SQLAlchemy removed.) |
