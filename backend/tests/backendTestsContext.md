...
| File | Covers |
|---|---|
| `conftest.py` | Points the app at a SEPARATE Mongo database `<MONGODB_DB>_test` (env set BEFORE app import — the real `fluently` DB is never touched). `fresh_db` autouse fixture wipes all collections before+after each test and re-ensures indexes. NO `db` fixture anymore (no SQLAlchemy). `client` (TestClient) fixture. `FakeChatModel` (scripted AIMessage responses, records invocations), `FakeStructuredFactory/Model` (canned structured outputs keyed by schema class — includes `topic_service.OnboardingFacts`). `mock_llms` autouse fixture patches all LLM factories unless test marked `@pytest.mark.live`. NOTE: mocked tests still hit REAL Atlas (the `_test` DB), so a run needs network + a valid `MONGODB_URI`. |
