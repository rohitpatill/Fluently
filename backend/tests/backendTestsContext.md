...
| `test_chat.py` | Basic turn, 400/404, tool loop + stored tool_calls transparency (`memory_update` action='append'), history reconstruction (AIMessage+ToolMessage pair with original id), judge events applied, judge failure swallowed, auto-title, agent adjust_word_score (+unknown id), opener (+400 when non-empty). |
