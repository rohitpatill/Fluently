...
| `android/` | **The native Android app** (Capacitor Gradle project) — committed as source because it carries the hand-edited manifest (OAuth deep link + mic permissions), the debug-only cleartext config, and the generated icons/splash. See [android/frontendAndroidContext.md](android/frontendAndroidContext.md), and [android/RELEASE.md](android/RELEASE.md) to ship a new Play Store version. |
