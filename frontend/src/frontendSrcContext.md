...
| `App.jsx` | Shell + gating order: health check (error ⇒ full-screen retry) → **auth gate** (`useMe`; 401/no session ⇒ `<Login>`) → onboarding gate (`me.has_persona` false ⇒ `<Onboarding>`) → responsive rail/bottom-nav + AnimatePresence view switch: 'chat' \| 'words' \| 'memory' \| 'settings'. Passes `me` to Rail/SettingsView. Persona/identity queries are `enabled` only once authenticated. |
